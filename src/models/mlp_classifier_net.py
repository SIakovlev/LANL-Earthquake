from torch.nn import Module, Linear, CrossEntropyLoss, ReLU, ModuleList, Dropout, BatchNorm1d
...
class MLP_classifier(Module, ModelBase):
    def __init__(self, **kwargs):
        ...
        if isinstance(self.in_features_shape, list) and len(self.in_features_shape) == 2:
            ...
            raise TypeError("input features parameter must either be an int value or a list of ints")
    ...
    def fit(self, train_data, train_y):
        ...
        train_y = torch.tensor(train_y.values.astype(np.long)).view(-1, 1).to(self.device)
        ...
        for e in range(self.num_epochs):
            ...
            for i in range(n_train_steps_per_epoch):
                ...
                loss = self.loss(predict, y_batch.squeeze())
                loss.backward()
                self.optim.step()

                _, predictions = torch.max(predict, 1)
                mae_loss = torch.abs(predictions.view(-1, 1) - y_batch).type(torch.FloatTensor).mean()
                print(f"\r step: {i} | cross entropy loss={loss.detach().cpu():.4f} | mae_loss={mae_loss.detach().cpu():.4f}",
                      end="")
            print()
            # validate
    ...
    def predict(self, test_data):
        ...
        _, y_predict = torch.max(self(test_data), 1)
        result = y_predict.detach().cpu().numpy()
        del test_data
        return np.random.normal(result + 0.5, 0.3)
